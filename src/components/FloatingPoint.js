...
class FloatingPoint extends Component {
    ...
    handleIEEE = () => {
        ...
        console.log(e)
        for (let i = binary.length; i <= 23; i ++) {
            mantissa.push(0);
        }
    ...
    render() {
        return (
            <ScrollView style={{ backgroundColor: color.gray, flex: 1 }} >
                <Form style={{ flexDirection: 'row', flex: 1,  flexDirection:'column' }} >
                    <Item>
                        <Input
                            ...
                            onChangeText={number => this.setState({ ...this.INITIAL_STATE, number })}
                        />
